...
int vmsAnimatedGIF::GetNextBlockLen()
{
	...
	case BLOCK_IMAGE:
		...
		int nLCTSize = (int)
			...
			(long double)(pow((long double)2, (long double)(pIDescr->GetPackedValue(ID_PACKED_LOCALCTSIZE)+1))));
...
BOOL vmsAnimatedGIF::Load()
{
	...
	if (m_pGIFLSDescriptor->GetPackedValue(LSD_PACKED_GLOBALCT) == 1)
	{
		...
		m_nGlobalCTSize = static_cast<int>
			(3* (long double)(pow((long double)2, (long double)(m_pGIFLSDescriptor->GetPackedValue(LSD_PACKED_GLOBALCTSIZE)+1))));
