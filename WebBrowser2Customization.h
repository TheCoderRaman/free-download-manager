...
#define AFX_WEBBROWSER2CUSTOMIZATION_H__E751A9D3_85F5_413D_83BC_EAB2DC14008F__INCLUDED_
...
class CWebBrowser2Customization : public CCmdTarget,
	protected IDocHostUIHandler
{
public:
	CWebBrowser2Customization();
	virtual ~CWebBrowser2Customization();
...
public:
	...
	//{{AFX_VIRTUAL(CWebBrowser2Customization)
	//}}AFX_VIRTUAL

protected:
	void OnDocumentComplete (LPDISPATCH pdWB, VARIANT *URL);
	void OnBeforeNavigate2 (LPDISPATCH, VARIANT *vtUrl, VARIANT *, VARIANT *vtTarget, VARIANT *, VARIANT *, BOOL *pbCancel);

	
	//{{AFX_MSG(CWebBrowser2Customization)
...
public:
	void Detach();
	HRESULT Attach (class CWebBrowser2 *pWB);
	DWORD getHostUiInfoFlags () const;
	void setHostUiInfoFlags (DWORD dw);

protected:
	
	
	virtual BOOL OnNavigateURL (LPCTSTR ptszUrl);
	CWebBrowser2* m_pWB;
	DWORD m_dwCookie;
	IConnectionPointPtr m_spCP;
	DWORD m_dwHostUiInfoFlags;
	long m_cRef;
};
